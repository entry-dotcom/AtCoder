...
int main() {
    ...
    int a, b, c, d, e, f, x, taka_dis = 0, aoki_dis = 0, aoki_nokori = 0, taka_nokori = 0, aoki_shuki, taka_shuki;
    cin >> a >> b >> c >> d >> e >> f >> x;
    // taka_nokori = x - x/(a+c)*(a+c);
    // aoki_nokori = x - x/(d+f)*(d+f);
    taka_shuki = x/(a+c);
    aoki_shuki = x/(d+f);
    taka_dis = a*taka_shuki*b + min(a,x%(a+c)) * b;
    aoki_dis = d*aoki_shuki*e + min(d,x%(d+f)) * e;
