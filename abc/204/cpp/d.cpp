...
int main() {
    ...
    vector<vector<int>> using_oven(2,vector<int> (100,0));
    ll sum = accumulate(t.begin(),t.end(),0);
    ll harf = (sum+1)/2;
    
    //sort(t.rbegin(),t.rend());
    using_oven.at(0).at(0) = t.at(0);
    using_oven.at(0).at(1) = t.at(1);

    rep(i,2,n) {
        using_oven.at(0).at(i) = min(using_oven.at(0).at(i-1),)
    }

    cout << max(oven1,oven2) << "\n";
