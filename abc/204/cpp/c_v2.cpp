...
int main() {
    ...
    rep(i,0,m) {
        //graph.at(b.at(i)-1).push_back(a.at(i)-1);
        graph.at(a.at(i)-1).push_back(b.at(i)-1);
    }
    ...
    rep(i,0,n) {
        ...
        while(!st.empty()) {
            ...
            st.pop();
            visit.at(v) = true;
            rep(i,0,(int)graph.at(v).size()) {
                if (!visit.at(graph.at(v).at(i))) {
                    visit.at(graph.at(v).at(i)) = true;
                    st.push(graph.at(v).at(i));
                    ans++;
                }
            }
        }
