...
int main() {
    ...
    int n, m, num = 0;
    cin >> n >> m;
    vi2 a(m,vector<int>(n));
    vector<vector<bool>> ans(n,vector<bool>(n,false));
    

    rep(i,0,m) {
        rep(j,0,n) cin >> a.at(i).at(j);
    }

    rep(i,0,m) {
        rep(j,0,n-1) {
            ans.at(a.at(i).at(j)-1).at(a.at(i).at(j+1)-1) = true;
            ans.at(a.at(i).at(j+1)-1).at(a.at(i).at(j)-1) = true;
        }
    }

    rep(i,0,n) {
        rep(j,i+1,n) if (!ans.at(i).at(j)) num++;
    }
