...
int main() {
    ...
    rep(i,0,h) {
        rep(j,0,w) {
            if (s.at(i).at(j)=='.' || visit.at(i).at(j)) continue;
            //visit.at(i).at(j) = true;
            stack<pair<int,int>> st;
            st.push({i,j});
            while(!st.empty()) {
                int dx = st.top().first, dy = st.top().second;
                st.pop();
                rep(k,0,8) {
                    int dh = dx + di.at(k); int dw = dy + dj.at(k);
                    if (0<=dh && dh<h && 0<=dw && dw<w) {
                        if (s.at(dh).at(dw)=='#' && !visit.at(dh).at(dw)) {
                            visit.at(dh).at(dw) = true;
                            st.push({dh,dw});
                        }
                    }
                }
