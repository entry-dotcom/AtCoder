...
int main() {
    ...
    ll n;
    cin >> n;
    vector<pair<ll,ll>> t_d(n);
    rep(i,0,n) {
        cin >> t_d.at(i).first >> t_d.at(i).second;
        t_d.at(i).second += t_d.at(i).first;
    }

    sort(t_d.begin(), t_d.end());
    int ans = 0, it = 0;
    ...
    for (ll i=0;; i++) {
        ...
        while (it<n && t_d.at(it).first==i) pq.push(t_d.at(it++).second);
        while(pq.top()<i && !pq.empty()) pq.pop();
        if (!pq.empty()) {
            pq.pop();
            ans++;
        }
