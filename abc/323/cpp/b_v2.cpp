...
int main() {
    ...
    rep(i,0,n) ans.at(i).second = i+1;

    rep(i,0,n) {
        rep(j,0,n) if (s.at(i).at(j)=='o') ans.at(i).first++;
    }
    ...
    rep(i,0,n) cout << ans.at(i).second << (i<n-1 ? " ":"\n");
    // for_each(ans.begin(),ans.end(), [] pair<int,int> ele) {cout << ele << "\n";}
