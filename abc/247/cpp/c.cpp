...
vector<int> ans(1,1);
vector<int> saiki(int n) {
    if (n==1) return ans;
    else {
        vector<int> kari_ans = ans;
        ans.push_back(kari_ans.at(kari_ans.size()/2)+1);
        rep(i,0,kari_ans.size()) ans.push_back(kari_ans.at(i));
        return saiki(n-1);
    }
}
